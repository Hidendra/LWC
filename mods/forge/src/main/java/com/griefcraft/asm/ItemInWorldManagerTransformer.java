            ...
            if (m.name.equals(hm.get("targetMethodName")) && m.desc.equals(targetMethodDesc))
            {
                ...
                for (int index = 0; index < m.instructions.size(); index++)
                {
                    ...
                    if (m.instructions.get(index).getOpcode() == IFNULL)
                    {
                        ...
                        toInject.add(new MethodInsnNode(INVOKESTATIC, "com/griefcraft/ForgeEventHelper", "onBlockHarvested", "(L" + hm.get("worldJavaClassName") + ";IIIL" + hm.get("blockJavaClassName") + ";IL" + hm.get("entityPlayerJavaClassName") + ";)Z"));
