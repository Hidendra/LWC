...
public class LWCListener extends PluginListener {
	...
	public boolean onBlockDestroy(Player player, Block block) {
		...
		if (chest_ != null) {
			...
			if (requestInfo) {
				...
				if (chest_.getType() == ChestTypes.PASSWORD) {
					...
					/*
					 * Players authed to the chest are still in the game-- let's
					 * colour them their prefix!:D
					 */
				...
				if (!canAccess) {
					player.sendMessage(Colors.Red
							+ "You cannot use a chest that you cannot access as a drop transfer target.");
					player.sendMessage(Colors.Red
							+ "If this is a passworded chest, please unlock it before retrying.");
					player.sendMessage(Colors.Red
							+ "Use \"/lwc droptransfer select\" to try again.");
				} else {
					MemoryDatabase.getInstance().registerMode(player.getName(), "dropTransfer", "f"
							+ chest_.getID());
					player.sendMessage(Colors.Green
							+ "Successfully registered chest as drop transfer target.");
				}
				...
				if (parent.canAdminChest(player, chest_)) {
					...
					for (String entity : entities) {
						...
						if (!remove) {
							...
							PhysicalDatabase.getInstance().registerRights(chestID, entity, isAdmin ? 1
									: 0, chestType);
							player.sendMessage(Colors.LightGreen
		...
		if (dropTransferReg) {
			player.sendMessage(Colors.Red
					+ "Cannot select unregistered chest as drop transfer target.");
			player.sendMessage(Colors.Red
					+ "Use \"/lwc droptransfer select\" to try again.");
			MemoryDatabase.getInstance().unregisterAllActions(player.getName()); // ignore
			// persist
			...
			if (createChest) {
				...
				if (type.equals("public")) {
					...
					for (String entity : entities) {
						...
						PhysicalDatabase.getInstance().registerRights(PhysicalDatabase.getInstance().loadChest(block.getX(), block.getY(), block.getZ()).getID(), entity, isAdmin ? 1
								: 0, chestType);
						player.sendMessage(Colors.LightGreen
								+ "Registered rights for "
								+ Colors.Gold
		...
		if (command.equalsIgnoreCase("lwc")) {
			...
			if (action.equals("create")) {
				...
				if (split.length < 3) {
					...
					player.sendMessage("");
					player.sendMessage(Colors.LightGreen
							+ "/lwc droptransfer select - Select a chest to drop transfer to");
					player.sendMessage(Colors.LightGreen
							+ "/lwc droptransfer on - Turn on drop transferring");
					player.sendMessage(Colors.LightGreen
							+ "/lwc droptransfer off - Turn off drop transferring");
					player.sendMessage(Colors.LightGreen
							+ "/lwc droptransfer status - Turn off drop transferring");
					return true;
				}
				...
				if (subAction.equals("select")) {
					if (parent.playerIsDropTransferring(playerName)) {
						player.sendMessage(Colors.Red
								+ "Please turn off drop transfer before reselecting a chest.");
						return true;
					}
					...
					player.sendMessage(Colors.Green
							+ "Please left-click a registered chest to set as your transfer target.");
				} else if (subAction.equals("on")) {
					int target = parent.getPlayerDropTransferTarget(playerName);
					if (target == -1) {
						player.sendMessage(Colors.Red
								+ "Please register a chest before turning drop transfer on.");
						return true;
					}
					...
					player.sendMessage(Colors.Green
							+ "Drop transfer is now on.");
					player.sendMessage(Colors.Green
							+ "Any items dropped will be transferred to your chest.");
				} else if (subAction.equals("off")) {
					int target = parent.getPlayerDropTransferTarget(playerName);
					if (target == -1) {
						player.sendMessage(Colors.Red
								+ "Please register a chest before turning drop transfer off.");
						return true;
					}
					...
					player.sendMessage(Colors.Green
							+ "Drop transfer is now off.");
				} else if (subAction.equals("status")) {
					if (parent.getPlayerDropTransferTarget(playerName) == -1) {
						player.sendMessage(Colors.Green
								+ "You have not registered a drop transfer target.");
						return true;
					} else {
						if (parent.playerIsDropTransferring(playerName)) {
							player.sendMessage(Colors.Green
									+ "Drop transfer is currently active.");
						} else {
							player.sendMessage(Colors.Green
									+ "Drop transfer is currently inactive.");
						}
					}
				}
				return true;
				...
				if (subAction.equalsIgnoreCase("clear")) {
					...
					for (int i = 4; i < split.length; i++) {
						...
						if (limit != -2) {
							PhysicalDatabase.getInstance().registerLimit(isGroup ? 0
									: 1, limit, entity);
							player.sendMessage(Colors.Green
							...
							PhysicalDatabase.getInstance().unregisterLimit(isGroup ? 0
									: 1, entity);
							player.sendMessage(Colors.Green
				...
				if (split.length < 3) {
					...
					player.sendMessage("");
					player.sendMessage(Colors.Green
							+ "This command converts chests of other plugins to LWC.");
					player.sendMessage("");
					player.sendMessage(Colors.LightGreen
							+ "/lwc convert <OtherPlugin>");
					player.sendMessage("");
					player.sendMessage("Currently supported: " + Colors.Gold
							+ "chestprotect");
				}
	...
	public boolean onItemDrop(Player player, Item item) {
		...
		if (!PhysicalDatabase.getInstance().doesChestExist(targetId)) {
			player.sendMessage(Colors.Red
					+ "Your drop transfer target was unregistered and/or destroyed.");
			player.sendMessage(Colors.Red
					+ "Please reregister a target. Drop transfer will be deactivated.");
		...
		if (chest_ == null) {
			player.sendMessage(Colors.Red
					+ "An unknown error occured. Drop transfer will be deactivated.");
			md.unregisterMode(pn, "dropTransfer");
		...
		if (!parent.canAccessChest(player, chest_)) {
			player.sendMessage(Colors.Red
					+ "You have lost access to your target chest.");
			player.sendMessage(Colors.Red
					+ "Please reregister a target. Drop transfer will be deactivated.");
		...
		for (Chest chest : chests) {
			Item toStack;
			while (((toStack = chest.getItemFromId(item.getItemId(), 63)) != null || chest.getEmptySlot() != -1)
					&& remainingAmt > 0) {
				if (toStack != null) {
					int amtDelta = Math.min(64 - toStack.getAmount(), item.getAmount());
					chest.setSlot(item.getItemId(), toStack.getAmount()
							+ amtDelta, toStack.getSlot());
		...
		if (remainingAmt > 0) {
			player.sendMessage(Colors.Red
					+ "Your chest is full. Drop transfer will be deactivated.");
			player.sendMessage(Colors.Red
					+ "Any remaining quantity that could not be stored will be returned.");
			md.unregisterMode(pn, "dropTransfer");
			md.registerMode(pn, "dropTransfer", "f" + targetId);
